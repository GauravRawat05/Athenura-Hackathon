...
const mapDbHackathon = (h) => {
  try {
    console.log("Mapping Hackathon Data:", h);
    if (!h) return null;
    ...
    const currencySymbol = h.currency === 'INR' ? '₹' : '$';
    
    // We can't know the exact fee without knowing if they are solo or team, 
    // but we can pass the raw data for the UI to handle.
    return {
      id: h._id,
      ...
      modes: h.allowedModes || ["solo"], // Array of modes
      prize: prizePoolNum,
      currency: currencySymbol,
      fees: { solo: h.soloFee || 0, team: h.teamFee || 0, intern: h.internFee || 0 },
      domain: domain,
      ...
      teamSize: { min: Number(h.minTeamSize) || 1, max: Number(h.maxTeamSize) || 4 },
      participants: Number(h.participantsCount) || 0,
      image: image,
      sponsors: Array.isArray(h.sponsors) ? h.sponsors.map(s => typeof s === 'string' ? s : s?.name || "Sponsor").filter(Boolean) : [],
      ...
      prizes: [
        { place: "1st", amount: `${currencySymbol}${(prizePoolNum * 0.6).toLocaleString()}`, perks: "Incubation Support" },
        { place: "2nd", amount: `${currencySymbol}${(prizePoolNum * 0.3).toLocaleString()}`, perks: "Cloud Credits" },
        { place: "3rd", amount: `${currencySymbol}${(prizePoolNum * 0.1).toLocaleString()}`, perks: "Swag Pack" }
      ],
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          >
            <div style={{ textAlign: "center", marginBottom: 20 }}>
              ...
              >
                {h.currency}{h.prize.toLocaleString()}
              </div>
              <div
                style={{
                  fontSize: 12,
          ...
          >
            {[
              ...
              },
              {
                icon: "🎯",
                label: "Mode",
                value: h.modes?.map(m => m.charAt(0).toUpperCase() + m.slice(1)).join(" & ") || "Solo",
              },
            ].map((item, i, arr) => (
              <div
                key={item.label}
